...
import { AtimeusProjectDto, AtimeusProjectSearchParams, atimeusServerService } from "@/lib/services/server/atimeus";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const paginationParams: AtimeusProjectSearchParams = {
      filter: query,
      page,
      size,
      sortBy,
    ...
    const data: ServerHateoasResponse<AtimeusProjectDto> = await atimeusServerService.getAllProjects(token, paginationParams);
    const response: ApiResponse<ServerHateoasResponse<AtimeusProjectDto>> = {
      status: 200,
      message: "Success",
      data: data,
