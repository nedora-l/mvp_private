...
import { AppDepartment, AppEmployee, AppTeam } from "@/lib/interfaces/apis/employee";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data:HateoasResponse<AppTeam>  = await directoryServerService.getTeams(token, params);
    const response: ApiResponse<HateoasResponse<AppTeam>> = {
      status: 200,
      message: "Success",
      data: data,
      type: "RECORD_DETAILS"
