...
/**
 * Get comments with replies for a file (nested structure)
 * GET /api/v1/files/{id}/comments/with-replies
 */
export async function GET(
  ...
  try {
    ...
    const commentsWithReplies: FileCommentDto[] = await filesServerService.getFileCommentsWithReplies(
      token, 
      params.id, 
      pagination
    );
    ...
    console.error("Get file comments with replies error:", error);
    return NextResponse.json(
      { 
        status: 500,
