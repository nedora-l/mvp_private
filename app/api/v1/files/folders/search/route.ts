...
export async function GET(request: NextRequest) {
  try {
    ...
    const searchFoldersData: HateoasResponse<FileFolderDto> = await filesServerService.searchFoldersFiles(token, params);
    const response: ApiResponse<HateoasResponse<FileFolderDto>> = {
      status: 200,
      message: "Success",
      data: searchFoldersData,
      type: "RECORD_DETAILS"
    };
    return NextResponse.json(response, { status: 200 });
  } catch (error) {
    console.error("Search folders files error:", error);
    return NextResponse.json(
      { 
        status: 500,
