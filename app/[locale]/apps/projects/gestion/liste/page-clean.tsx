...
i  const handleDelete = async (id: string) => {
    if (!window.confirm("Supprimer ce projet ?")) return;
    setDeletingId(id);
    setError(null);
    try {
      // Trouver l'index du projet à partir de l'ID
      const index = projects.findIndex(p => p.id === id);
      if (index === -1) throw new Error("Projet non trouvé");
      
      await deleteProject(index); // Utiliser l'index pour l'API
  ...
  };t, useProjects } from "@/contexts/projects-context";
...
export default function ListeDesProjetsPage() {
  ...
  const handleAddProjectLocal = async (form: { name: string; type: string; description: string }) => {
    setAdding(true);
    try {
  ...
  const handleEdit = async (id: string, form: { name: string; type: string; description: string; status?: string; startDate?: string; endDate?: string; members?: string; customType?: string }) => {
    setEditingId(id);
    setError(null);
    try {
      // Trouver l'index du projet à partir de l'ID
      ...
      await editProject(index, updatedProject); // Utiliser l'index pour l'API
      setEditingId(null);
      ...
      await editProject(parseInt(id), updatedProject); // Utiliser la fonction du contexte
      setEditingId(null);
      console.log("✅ Projet modifié via contexte");
    } catch (error) {
      console.error("❌ Erreur lors de la modification:", error);
