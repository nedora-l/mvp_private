...
import { formatCurrency, getPluralForm } from "@/lib/i18n/utils"
...
export function List02({ dictionary, locale }: List02Props) {
  // Use our pluralization utility to create a transactions count message
  const transactionText = getPluralForm(transactions.length, locale, {
    one: dictionary.plurals.items.singular,
    other: dictionary.plurals.items.plural
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
          ...
          {dictionary.documents.title || "Recent Transactions"}
        </h2>
        <span className="text-xs text-gray-500 dark:text-gray-400">{transactionText}</span>
      </div>
          ...
          <div key={transaction.id} className="flex items-center justify-between">
            ...
            <div className="flex items-center">
              ...
              >
                ...
                {formatCurrency(Math.abs(transaction.amount), locale, 'USD')}
              </span>
              {transaction.type === "income" ? (
      ...
      <button className="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
        {dictionary.common.seeAll || "View All Transactions"}
      </button>
    </div>
  )
}
